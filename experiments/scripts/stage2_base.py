...
def run_experiment(config_path):
    """Run Stage2 experiment with MLflow tracking."""
    ...
    with mlflow.start_run(run_name=config.get('run_name')):
        ...
        # Visualize results
        vis_dir = os.path.join("outputs", "visualizations", config.get('run_name'))
        visualize_and_save_samples(best_model, dataloaders['val'], vis_dir)
        mlflow.log_artifacts(vis_dir, "visualizations")
        
        # Save model
        model_dir = os.path.join("outputs", "models", config.get('run_name'))
        ...
        if s3_bucket:
            s3_prefix = f"experiments/{config.get('experiment_name')}/{config.get('run_name')}"
